...
const getRGB = (key) => {
  ...
  // Handle dynamic CSS values like [#ff0000]
  if (/^\[#[0-9A-Fa-f]{3,6}\]$/.test(key)) {
    const hex = key.replace(/[\[\]]/g, ""); // Remove brackets
    return hexToRGB(hex);
  }

  // Handle direct hex codes like #ff5733
  if (/^#[0-9A-Fa-f]{3,6}$/.test(key)) {
    return hexToRGB(key);
  }

  // Handle other bracketed values (e.g., custom RGB input)
  if (/^\[.*?\]$/.test(key)) {
    return key.replace(/[\[\]]/g, "");
  }

  return null; // Return null if the key isn't recognized
};
module.exports = {
  getRGB,
};
